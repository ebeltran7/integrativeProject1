...
public class frmCrearReservaSala extends javax.swing.JDialog {
    ...
    private frmAdministrador admin;
    private Usuario usuarioActual; // ← Añadir esta línea
    private Date fecha;
    private Date horaInicio;
    private Date horaFin;
    private Sala salaSeleccionada;
    
    public frmCrearReservaSala(frmAdministrador parent, boolean modal, String rolUsuario) {
        
        super(parent, modal);
        initComponents();
        this.usuarioActual = usuarioActual;
        this.admin = parent; // guarda la referencia al formulario principal
    ...
    private void crearReservaSala() {
        try {
            ...
            if (filas > 0) {
                ...
                admin.cargarReservasSala();   // Actualiza tabla en frmAdministrador 
...
private void guardarReserva() {
    try {
        // Asegúrate que tienes la conexión abierta
        usuarioDAO dao = new usuarioDAO();
        dao.conectar();
        ...
        // Combina fecha y hora para comparar con fecha/hora actual
        Calendar calInicio = Calendar.getInstance();
        calInicio.setTime(fecha);
        Calendar calHoraInicio = Calendar.getInstance();
        calHoraInicio.setTime(horaInicio);
        calInicio.set(Calendar.HOUR_OF_DAY, calHoraInicio.get(Calendar.HOUR_OF_DAY));
        calInicio.set(Calendar.MINUTE, calHoraInicio.get(Calendar.MINUTE));
        Date fechaHoraInicio = calInicio.getTime();

        Date ahora = new Date();

        // Validaciones
        if (fechaHoraInicio.before(ahora)) {
            JOptionPane.showMessageDialog(this, "La fecha y hora de inicio deben ser futuras.");
            return;
        }
        ...
        // Aquí obtén el id de la sala seleccionada (asegúrate que getIdSalaSeleccionada() funciona)
        int idSala = getIdSalaSeleccionada();

        // Aquí debes obtener el ID de la solicitud o usuario actual.
        // Asumamos que tienes un objeto usuarioActual definido en el formulario:
        int idUsuario = usuarioActual.getId();

        // Primero crea la solicitud en la tabla SOLICITUD, para relacionar luego con RESERVA
        int idSolicitud = dao.insertarSolicitud(idUsuario, "RESERVA_SALA", new Date(), "PENDIENTE", null);
        ...
        // Inserta la reserva con el ID_SOLICITUD generado
        ...
        if (filas > 0) {
            JOptionPane.showMessageDialog(this, "Reserva de sala creada exitosamente.");
            // Actualiza la tabla de reservas en frmAdministrador
            if (admin != null) {
                admin.cargarReservasSala();
            }
            this.dispose();
        } else {
            JOptionPane.showMessageDialog(this, "Error al crear la reserva.");
...
public static void main(String args[]) {
    ...
    java.awt.EventQueue.invokeLater(new Runnable() {
        public void run() {
            ...
            usuarioActual.setRol("Administrador");     

            frmAdministrador admin = new frmAdministrador(usuarioActual);
            String rolActual = "Administrador" ;
            frmCrearReservaSala dialog = new frmCrearReservaSala(admin, true, rolActual);
